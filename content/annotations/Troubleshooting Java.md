...
<font color="#EF7DFA"><strong>p.55</strong> "ProductService saveProduct() DAE ProductController 바로 호출하지 않았다는 사실이 밝혀졌다. 두 메서드 사이에서 애스펙트가 실행된 것"</font>
...
<font color="#EF7DFA"><strong>p.59</strong> "스텝 인투를 할 때마다 조  사 플랜이 새로 열리면 전체 프로세스가 점점 복잡해진다("</font>
...
## 2 실행을 중단시키지 않고도 브레이크포인트를 사용  하는 방법
---
...
<font color="#EF7DFA"><strong>p.151</strong> "DB에서 다량의 레코드를 가져와 정해진 방식으로 처리하는 예약 프로세스가 있었는데,  언젠가부터 이 프로세스가 가져오는 레코드 수가 너무 많아지면서 그 자체만으로 메타스페이스  가 꽉 차버린 것"</font>
...
## 3.2 프로파일러로 프레임워크에서 생성된 SQL 쿼리 식별
---
...
**p.172** "이런 종류의 문제를 조사할 때 로그를 사용하면 일단 어떤 쿼리가 문제인지 식별하기가 어렵다는  단점이 있다."


**p.172** "da-ch7-ex3 프로젝트의 설정 프로퍼티 파일에 다음 매개변수를 추가하면 하이버네이트에서 생성  된 쿼리를 로그에 출력하도록 구성할 수 있다."


**p.173** "레이턴시 문제를 조사할 때는 언제나 프로파일러로 시작하는 것이 좋다. 첫 번째 단계는 샘플링이다. SQL 쿼  리 문제가 의심된다면 JDBC에 관한 프로파일링을 수행하라."

 
## 733 프로파일러로 프로그램에서 생성된 SQL 쿼리 식별
---
...
**p.174** "쿼리 프로  파일링을 개발 프로세스의 일부분으로 습관화하라고 개발자들에게 조언한다."
