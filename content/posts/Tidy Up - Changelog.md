---
title: Tidy Up - Changelog
description: 
draft: false
tags:
  - series/tidy-up
aliases: 
permalink: 
date: 2025-07-14
---
A [changelog](https://en.wikipedia.org/wiki/Changelog) for posts with #series/tidy-up

2025-07-12
- Change pageTitle
- Add `RecentNote` sections for Posts / Fleeting Notes
- Make tags not displayed in `RecentNote`

2025-07-10
- Add a `Comments` section by [Giscus](https://giscus.app/)
- Change a header font to `DM Serif Display`
- Make `Comments` section for a single page, not shared pages
- Fix links for images (attachments)

