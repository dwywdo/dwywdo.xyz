...
> [!COMMENT]- Where should we put `@EnableCommand` or `@CommandScan` annotations?
...
> [!COMMENT]- If you specify the A.class in `@EnableCommand` , It finds the class A's bean with `@Command` annotation from all `@Configuration` classes.
